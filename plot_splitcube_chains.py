...
burnin = 70
ndim = 3
chains = np.load('splitcube_chains.npy')
samples = chains[:, burnin:, :].reshape((-1, ndim))

prenams = ['centx', 'centy', 'voffset']
...
fig = corner.corner(samples, bins=[50, 50, 50], levels=levels, plot_datapoints=False, fill_contours=True, smooth=1,
...
mcmc_par0, mcmc_par1, mcmc_par2 = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))

print("""MCMC result:
    par0 = {0[0]} +{0[1]} -{0[2]})
    par1 = {1[0]} +{1[1]} -{1[2]})
    par2 = {2[0]} +{2[1]} -{2[2]})
""".format(mcmc_par0, mcmc_par1, mcmc_par2))
